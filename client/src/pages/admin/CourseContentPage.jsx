  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-4 p-3 bg-white/50 rounded-xl hover:bg-white/80 transition-colors border border-transparent hover:border-clay-accent/10 group mb-2">
      ...
      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => onEdit(lesson, isQuiz, isPdf)} className="w-8 h-8 rounded-lg bg-white text-blue-600 flex items-center justify-center shadow-sm hover:scale-105 transition-transform" title="تعديل الدرس" aria-label="تعديل الدرس"><Edit size={16} /></button>
        <button onClick={() => onDelete(lesson)} className="w-8 h-8 rounded-lg bg-white text-red-600 flex items-center justify-center shadow-sm hover:scale-105 transition-transform" title="حذف الدرس" aria-label="حذف الدرس"><Trash2 size={16} /></button>
      </div>
    </div>
  )
}
              ...
              <div className="bg-gradient-to-r from-clay-accent/10 to-transparent p-4 flex items-center justify-between border-b border-clay-accent/10">
                ...
                <div className="flex gap-2">
                  <button onClick={() => {
                    ...
                    setShowUnitModal(true)
                  }} className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition-colors" title="تعديل الوحدة" aria-label="تعديل الوحدة"><Edit size={16} /></button>
                  ...
                  }} className="w-8 h-8 rounded-lg bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-100 transition-colors" title="حذف الوحدة" aria-label="حذف الوحدة"><Trash2 size={16} /></button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          ...
          >
            ...
            <div className="p-6 border-b border-clay-accent/10 flex items-center justify-between flex-shrink-0 relative z-10">
              ...
              <button type="button" onClick={() => setShowLessonModal(false)} className="text-clay-muted hover:text-red-500 transition-colors" aria-label="إغلاق">✕</button>
            </div>
              ...
              {selectedType === 'video' && (
                <div className="flex flex-col gap-2 p-4 bg-violet-50 rounded-2xl border border-violet-100">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <button type="button" onClick={() => document.getElementById('attachment-file').click()} disabled={uploading} className="p-3 rounded-xl bg-white text-clay-accent border border-clay-accent/20 hover:bg-clay-accent/5 transition-colors" aria-label="إضافة مرفق">
      ...
      {showUnitModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <ClayCard hover={false} className="w-full max-w-md shadow-2xl relative">
            <button onClick={() => setShowUnitModal(false)} className="absolute top-4 left-4 text-clay-muted hover:text-red-500" aria-label="إغلاق">✕</button>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-md">
          <ClayCard hover={false} className="w-full max-w-4xl shadow-2xl relative h-[85vh] flex flex-col !p-0 overflow-hidden">
            <div className="p-4 bg-amber-50 border-b border-amber-100 flex items-center justify-between flex-shrink-0">
              ...
              <button onClick={() => setShowQuizBuilder(false)} className="w-8 h-8 rounded-lg bg-white/50 text-amber-800 flex items-center justify-center hover:bg-white transition-colors" aria-label="إغلاق">✕</button>
            </div>
                  ...
                  {quizQuestions.map((q, idx) => (
                    <div key={q.id} className="bg-white rounded-2xl p-5 shadow-sm border border-clay-accent/10">
                      ...
                      <div className="flex gap-2 mt-4 justify-end pt-4 border-t border-slate-100">
                        <button onClick={() => handleDeleteQuestion(q.id)} className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-lg text-red-600 hover:bg-red-50 transition-colors" aria-label="حذف السؤال"><Trash2 size={14} /> حذف</button>
                      </div>
                    </div>
                  ))}
                </div>
