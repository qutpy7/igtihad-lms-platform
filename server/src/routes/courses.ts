...
router.put('/:id', authenticate, requireRole('admin'), async (req, res) => {
    try {
        ...
        const allowedFields = ['title', 'description', 'short_desc', 'grade', 'term', 'price', 'original_price', 'color', 'thumbnail_url', 'is_featured', 'is_active'];
...
router.put('/units/:unitId', authenticate, requireRole('admin'), async (req, res) => {
    try {
        const db = await getDb();
        const updates = req.body;
        const allowedFields = ['title', 'sort_order', 'thumbnail_url'];
        const keys = Object.keys(updates).filter(k => allowedFields.includes(k));
...
router.put('/lessons/:lessonId', authenticate, requireRole('admin'), async (req, res) => {
    try {
        const db = await getDb();
        const updates = req.body;
        const allowedFields = ['title', 'type', 'content_url', 'attachment_url', 'content', 'duration', 'sort_order', 'thumbnail_url', 'allow_retake'];
        const keys = Object.keys(updates).filter(k => allowedFields.includes(k));
